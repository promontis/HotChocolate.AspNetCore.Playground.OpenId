...
namespace HotChocolate.AspNetCore.Playground
{
    internal sealed class SettingsMiddleware
    {
            ...
            PlaygroundOptions options)
        {
            ...
            _subscriptionPath = uiPath.MakeRelativeUri(subscriptionPath).ToString();
        }
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            string callbackUrl = BuildUrl(_options.GraphQLEndpoint, context.Request, false, "callback");
            ...
            await context.Response.WriteAsync($@"
                window.Settings = {{
                    ...
                    path: ""{_options.Path}"",
                    opendIdConfiguration: {{
                        client_id: ""{_options.OpenIdConfiguration.ClientId}"",
                        redirect_uri: ""{callbackUrl}"",
                        response_type: ""{_options.OpenIdConfiguration.ResponseType}"",
                        scope: ""{_options.OpenIdConfiguration.Scope}"",
                        authority: ""{_options.OpenIdConfiguration.Authority}""
                    }}
                }}
            ",
